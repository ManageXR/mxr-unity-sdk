...
namespace MXR.SDK.Protocol.Tests {
    public class EnvelopeCodecTests {
        ...
        public void TryDecode_FrameError_DoesNotDeserializeEnvelope() {
            ...
            Assert.IsNull(envelope);
            Assert.AreEqual(nameof(FrameDecodeError.TruncatedFrame), error);
            Assert.AreEqual(FrameDecodeError.TruncatedFrame, frameResult.Error);
        }

        [Test]
        public void TryDecode_FrameViolation_ReportsNamedError() {
            ...
            Assert.IsFalse(success);
            Assert.IsNull(envelope);
            Assert.AreEqual(nameof(FrameDecodeError.ZeroLengthBody), error);
            Assert.AreEqual(FrameDecodeStatus.Violation, frameResult.Status);
        }
        ...
        [Test]
        public void TryDeserialize_InvalidUtf8_ReturnsFalse() {
            var bytes = new byte[] { 0xFF, 0xFE, 0x7B, 0x22, 0x76, 0x22, 0x3A, 0x32, 0x7D };

            Assert.IsFalse(_json.TryDeserialize(bytes, out _, out var error));
